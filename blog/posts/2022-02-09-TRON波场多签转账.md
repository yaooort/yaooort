...
async function createMultiTransaction() {
    ...
    var parameter = [{type: 'address', value: 'TNpVE2Nh1gywwFhnxU6aJGnKoe9NuxbRVx'}, {
        type: 'uint256',
        value: 1.2 * 1000000
    }];
    var options = {
    ...
    const signedTxAccount1 = await tronWeb.trx.multiSign(tx.transaction, privateKeyAccount1, 2); // 0 为ownerPermission， Witness ID = 1，  Active ID从2开始递增
    ...
    var signWeight = await tronWeb.trx.getSignWeight(signedTxAccount2);
    console.log("查询当前签名情况，Weight当前够不够去广播交易", signWeight)
    // 已签名账号列表
    var approvedList = await tronWeb.trx.getApprovedList(signedTxAccount2);
